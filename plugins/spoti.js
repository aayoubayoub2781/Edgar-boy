...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    if (!text) return await conn.reply(
        m.chat, 
        `ما الذي تبحث عنه؟ أدخل اسم الأغنية لتنزيلها من Spotify. مثال: *${usedPrefix + command} ozuna*`, 
        m, 
        {
            contextInfo: {
                externalAdReply: {
    ...
    try {
        ...
        if (!songInfo.length) throw `*لم يتم العثور على الأغنية.*`;
        let song = songInfo[0]; 
        const res = await fetch(`https://deliriussapi-oficial.vercel.app/download/spotifydl?url=${song.url}`);
        const data = await res.json();
        if (!data || !data.data || !data.data.url) throw "تعذر الحصول على رابط التنزيل.";
        let spotifyMessage = `*• العنوان:* ${song.name}\n*• الفنان:* ${song.artista.join(', ')}\n*• الغلاف:* ${data.data.cover}\n\n> 🚀 *جارٍ إرسال الأغنية، الرجاء الانتظار قليلاً...*`;
        ...
        try {
            ...
            m.reply(`\`\`\`⚠️ حدث خطأ ⚠️\`\`\`\n\n> *قم بالإبلاغ عن الخطأ باستخدام الأمر:* #report\n\n>>> ${error} <<<< `) 
            console.log(error) 
            m.react('❌')
        }
    }
